...
'''
    Algorithms for matrix LU, LUP decomposition, and simplex
...
def lupsolve(LU, pi, b):
    n = len(b)
    y = [float(b[pi[i]]) for i in range(n)]
    x = [0.0 for i in range(n)]

    for i in range(0, n):
        summation = 0.0
        for j in range(0, i):
            summation += LU[i][j] * y[j]
        y[i] -= summation

    for i in range(n - 1, -1, -1):
        summation = 0.0
        for j in range(i + 1, n):
            summation += LU[i][j] * x[j]
        x[i] = (y[i] - summation) / LU[i][i]
    return x
...
def pivot(A, row, col):
    m = len(A)
    n = len(A[0])
    Ahat = [[float(A[i][j]) for j in range(n)] for i in range(m)]

    pivot_element = float(A[row][col])

    for j in range(0, n):
        Ahat[row][j] = float(A[row][j]) / pivot_element

    for i in range(0, m):
        if i != row:
            for j in range(0, n):
                Ahat[i][j] -= (float(Ahat[row][j]) *
                               float(A[i][col]))
